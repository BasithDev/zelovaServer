...
const getIssues = async (req, res) => {
    try {
        ...
        return res.status(statusCodes.INTERNAL_SERVER_ERROR).json({
            success: false,
            message: MESSAGES.ISSUE.GENERIC_ERROR,
        });
    }
};
    ...
    try {
        ...
        if (!issue) {
            return res.status(statusCodes.NOT_FOUND).json({
                success: false,
                message: MESSAGES.ISSUE.NOT_FOUND,
            });
        }
        
        // Email sending is non-blocking - don't fail if email fails
        try {
        ...
        return res.status(statusCodes.OK).json({
            success: true,
            message: MESSAGES.ISSUE.RESOLVED_SUCCESS,
        });
    } catch (error) {
        console.error(error);
        return res.status(statusCodes.INTERNAL_SERVER_ERROR).json({
...
const ignoreIssue = async (req, res) => {
    try {
        ...
        return res.status(statusCodes.OK).json({
            success: true,
            message: MESSAGES.ISSUE.IGNORED_SUCCESS,
        });
    } catch (error) {
        console.error(error);
        return res.status(statusCodes.INTERNAL_SERVER_ERROR).json({
...
const refundUser = async (req, res) => {
    try {
        ...
        if (!userZcoin) {
            return res.status(statusCodes.NOT_FOUND).json({
                success: false,
                message: MESSAGES.ZCOIN.NOT_FOUND,
            });
        }
        ...
        return res.status(statusCodes.OK).json({
            success: true,
            message: MESSAGES.ISSUE.REFUND_SUCCESS,
        });
    } catch (error) {
        console.error(error);
        return res.status(statusCodes.INTERNAL_SERVER_ERROR).json({
...
const getOrderDetails = async (req, res) => {
    try {
        ...
        return res.status(statusCodes.INTERNAL_SERVER_ERROR).json({
            success: false,
            message: MESSAGES.ISSUE.ORDER_DETAILS_FETCH_FAILED
        });
    }
};
